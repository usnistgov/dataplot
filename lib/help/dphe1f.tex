7200.       (VERSION 2025.07)  total number of lines in file (including this line)
...
Supported statistics can be used in the following commands:
  ...
  19. <STAT> INTERACTION PLOT <STAT>
  20. CLASSIFICATION <STAT> CURVE
  21. <STAT> INFLUENCE CURVE
  22. CROSS TABULATE <STAT>
  23. POSITIONAL TABULATION <STAT>
  24. TWO SAMPLE <STAT> PERMUTATION TEST
  25. K SAMPLE <STAT> PERMUTATION TEST
  26. LET V = MATRIX COLUMN <STAT>
  27. LET V = MATRIX ROW <STAT>
  28. LET A = MATRIX GRAND <STAT>
  29. LET M = MATRIX PARTITION <STAT>
  30. LET M = GENERATE MATRIX <STAT>
  31. LET V = CROSS TABULATE <STAT>
  32. LET V = CROSS TABULATE CUMULATIVE <STAT>
  33. LET V = SORT BY <STAT>
  34. LET YOUT = MOVING <STAT>
  35. LET YOUT = CUMULATIVE <STAT>
  36. LET YOUT = WINDOW <STAT>
...
DISTRIBUTIONS REQUIRING THE PARAMETER SIGMA
...
DISTRIBUTIONS REQUIRING THE PARAMETERS L, K, N, M
   HYPERGEOMETRIC RANDOM NUMBER    Generate hypergeometric random
                                   numbers
...
topics are discussed:
  ...
  17. Reading numeric values with trailing "+" or "-" or leading
      "<" or "">"
...
direct reading of Excel data files.

One method for reading data from Excel is to save the data in Excel to
an ASCII file and read that ASCII file into Dataplot.
...
READING NUMERIC VALUES WITH TRAILING "+" OR "-" OR LEADING "<" OR ">"
...
Note: The 2025/04 version of Dataplot added a similar capability for
leading "<" (to indicate "less than") or leading ">" (to indicate
"greater than").  Specifically, the following commands were added
...
OFF setting, the leading "<" and ">" characters will be treated as errors.
For the MISSING VALUE setting, data values with a leading "<" or ">" are
set to the value defined by the command

        SET READ MISSING VALUE <value>

The default setting is MISSING VALUE.  Note that the MISSING VALUE option
was added to the SET READ TRAILING PLUS MINUS IGNORE command as well.

In order to allow you to identify which data values contain the "<" or ">"
characters, Dataplot writes variables to "dpst5f.dat" that correspond to the
columnns read (that is, if you read six variables, the dpst5f.dat file will
contain six columns).  The dpst5f.dat columns will contain either 0.0, 1.0
or 2.0 values.  A value of 0.0 indicates "<" or ">" are not present, a value
of 1.0 indicates a "<" and a value of 2.0 indicates a ">".

If you enter SET READ TRAILING PLUS MINUS IGNORE <ON/MISSING VALUE>, this
will also create the dpst5f.dat file with 1.0 indicating a trailing "-"
and a 2.0 indicating a trailing "+".
