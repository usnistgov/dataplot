...
-----------------------------------------------------------------------
 ...
 1. You can now use the Tikz package rather than the epic/eepic package
    for LaTeX graphs by entering the command

        SET LATEX DRIVER TIKZ

    Enter HELP LATEX for details.

 2. Added the following LET subcommands

        LET AVAL = FULL WIDTH HALF MAXIMUM Y X
...
-----------------------------------------------------------------------
 ...
 1. Added the following Analysis commands

        ORDINARY LEAST PRODUCTS FIT Y X
        PASSING BABLOK FIT Y X
        THIEL SEN FIT Y X
        KENDALL TAU B CONFIDENCE LIMITS Y1 Y2

 2. Added the following LET sub-commands

        LET IINDX = INDEX LESS THAN                 Y
        LET IINDX = INDEX LESS THAN OR EQUAL TO     Y
        LET IINDX = INDEX GREATER THAN              Y
        LET IINDX = INDEX GREATER THAN OR EQUAL TO  Y

 3. Added the following SET sub-commands

       SET READ TAB TO <COMMA/SPACE>
       SET READ NUMBERS CONTAIN <COMMA/UNDERSCORE/OFF>

    The SET READ TAB TO COMMA is useful for tab delimited data files
    with missing cells or unequal length columns.  It converts the
    tabs to commas and then treats the data file as a comma separated
    file.  The default is SPACE (i.e., a tabs are converted to spaces).

    The SET READ NUMBERS CONTAIN command can be used to allow numbers
    in a data file to contain either commas or underscores (but not both
    on the same line).  One reason for doing this is for clarity when
    entering large numbers (e.g., 1_231_659.26).  Another reason is
    that financial data may contain commas (e.g. $231,840.11).  The
    default is OFF (i.e., commas or underscores will be treated as
    errors).

 4. Made a number of update to the DEVICE LATEX and CAPTURE LATEX
    commands.  Enter HELP LATEX or HELP CAPTURE LATEX for details.

 5. Fixed a bug in the DEVICE GD command.  This command was temporarily
    broken in the last release.
...
-----------------------------------------------------------------------
 ...
 1. Converted the Fortran source code from fixed format to free
    format syntax.  This should not change how Dataplot operates.
    The motivation for this is a first step to making the source
    code more Fortan90 compliant and to make the incorporation of
    libraries that are free format easier.
 ...
 3. Added the following graphics commands
        ...
        COMMUTABILITY DIFFERENCE PLOT
        COMMUTABILITY PLOT

        DUMBBELL PLOT Y1 Y2
        WATERFALL CHART Y
        BULLET PLOT Y YTARG YRANGE
        ...
        DOT MATRIX PLOT            COUNT GROUP CATEGORY
        DOT MATRIX PROPORTION PLOT       GROUP CATEGORY
        DOT MATRIX PROPORTION PLOT COUNT GROUP CATEGORY

        RIDGELINE PLOT Y X

 4. Added the following LET statistic commands

        LET XPERC = <percentile> GROUPED PERCENTILE FREQ XMID
        LET XPERC = <percentile> GROUPED PERCENTILE FREQ XLOW XHIGH
        LET XQUAN = <quantile>   GROUPED QUANTILE   FREQ XMID
        LET XQUAN = <quantile>   GROUPED QUANTILE   FREQ XLOW XHIGH

        LET YPERC = PERCENT ACCEPTABLE Y

 5. Added the following LET math commands

        LET Y2 X2 = DEMING FIT Y X

        LET YNEW  = REMOVE Y YDELETE

 6. Added the following LET string commands

        LET SOUT = NUMBER TO CURRENCY VALUE

 7. On occassion, data files may contain leading "<" or ">" characters
    that signify the data is less than or greater than the given value.
    To ignore these characters, enter one of the following

       SET READ LEADING LESS THAN IGNORE <MISSING VALUE/ON/OFF>
       SET READ LEADING GREATER THAN IGNORE <MISSING VALUE/ON/OFF>
       SET READ LEADING LESS THAN GREATER THAN IGNORE <MISING VALUE/ON/OFF>

    Note that these are synonyms.  Any of the these will process both the
    "<" or the ">" case.

    The default is MISSING VALUE.  For this setting, data values with a
    leading "<" or ">" are set to the value defined by the command

        SET READ MISSING VALUE <value>

    In order to allow you to identify which data values contain the "<"
    or ">" characters, Dataplot writes variables to "dpst5f.dat" that
    correspond to the columnns read (that is, if you read six variables,
    the dpst5f.dat file will contain six columns).  The dpst5f.dat columns
    will contain either 0.0, 1.0 or 2.0 values.  A value of 0.0 indicates
    "<" or ">" are not present, a value of 1.0 indicates a "<" and a value
    of 2.0 indicates a ">".

    If you enter SET READ TRAILING PLUS MINUS IGNORE ON, this will also
    create the dpst5f.dat file with 1.0 indicating a trailing "-" and
    a 2.0 indicating a trailing "+".

    For the ON setting, the leading "<" or ">" is simply ignored.  For the
    OFF setting, the leading "<" and ">" characters and trailing "-" and
    "+" characters will be treated as errors.

    Note that there should be no space between the leading "<" or ">"  or
    the trailing "-" or "+" and the data value since Dataplot treats the
    space character as a delimiter.

 8. The TIC MARK OFFSET UNITS command was updated to allow the
    offset units to be specified separately for each of the four
    frame lines.  So you can use the following commands

        X1TIC MARK OFFSET UNITS <DATA/ABSOLUTE> - bottom frame
        X2TIC MARK OFFSET UNITS <DATA/ABSOLUTE> - top frame
        Y1TIC MARK OFFSET UNITS <DATA/ABSOLUTE> - left frame
        Y2TIC MARK OFFSET UNITS <DATA/ABSOLUTE> - right frame

        XTIC MARK OFFSET UNITS <DATA/ABSOLUTE>  - bottom and top frames
        YTIC MARK OFFSET UNITS <DATA/ABSOLUTE>  - left and right frames
        TIC MARK OFFSET UNITS <DATA/ABSOLUTE>   - all 4 frames

 9. The READ EXCEL command was updated to allow Powershell rather than
    Python/Pandas to be used to read Excel files.  This option is
    currently only supported for Windows platforms.  Enter HELP READ
    EXCEL for details.

    Updated the Python script for READ EXCEL to support specifying
    the first and last column to read (inclusive).
...
-----------------------------------------------------------------------
 ...
 5. A number of changes were made to address a compilation issue
 ...
 6. A number of miscellaneous bug fixes were made.  In particular,
