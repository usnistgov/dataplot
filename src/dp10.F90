...
33172         FORMAT(A1,'documentclass[',I1,'pt]{article}')
          ...
          WRITE(ICOUT,999)
          CALL DPWRST('XXX','WRIT')
          WRITE(ICOUT,33174)IBASLC
33174     FORMAT(A1,'usepackage[T1]{fontenc}')
          CALL DPWRST('XXX','WRIT')
          WRITE(ICOUT,33176)IBASLC
33176     FORMAT(A1,'usepackage[body={6.5in,8.5in}]{geometry}')
          CALL DPWRST('XXX','WRIT')
          WRITE(ICOUT,33178)IBASLC
33178     FORMAT(A1,'usepackage{amsmath}')
          CALL DPWRST('XXX','WRIT')
!
!         THE ILATCG (SET LATEX CAPTURE GRAPHICS) SPECIFIES WHETHER
!         GRAPHS WILL BE GENERATED BY LATEX (DEVICE 1 LATEX) OR
!         IMPORTED AS POSTSCRIPT/PDF FILES (DEVICE 2 POSTSCRIPT/
!         DEVICE 2 CLOSE SEQUENCE WITHIN THE CAPTURE).  SOME OF THE
...
23175       FORMAT(A1,'makeatother')
 ...
 3175       FORMAT(A1,'usepackage{graphics,color}')
            CALL DPWRST('XXX','WRIT')
            WRITE(ICOUT,3176)IBASLC
 3176       FORMAT(A1,'usepackage{graphicx}')
            CALL DPWRST('XXX','WRIT')
          ENDIF
!
...
13183     FORMAT(A1,'baselineskip=12pt')
          ...
          CALL DPWRST('XXX','WRIT')
!
!         2026/07: If LaTeX color turned on, add following block
!                  of code to define colors
!
          IF(ILATCG.EQ.'LATE')THEN
            AINC=0.010
            DO II=0,9
               AVAL=REAL(II)*AINC
               WRITE(ICOUT,14001)IBASLC,II,AVAL
14001          FORMAT(A1,'definecolor{G',I1,'}{gray}{',F5.3,'}')
               CALL DPWRST('XXX','WRIT')
            ENDDO
            DO II=10,99
               AVAL=REAL(II)*AINC
               WRITE(ICOUT,14111)IBASLC,II,AVAL
14111          FORMAT(A1,'definecolor{G',I2,'}{gray}{',F5.3,'}')
               CALL DPWRST('XXX','WRIT')
            ENDDO
...
!
 ...
 3181         FORMAT(A1,'newcommand{',A1,'PGRAPHIC}[1]{',A1,          &
                     'begin{figure}','[h]')
              CALL DPWRST('XXX','WRIT')
 ...
 3186         FORMAT(A1,'newcommand{',A1,'LGRAPHIC}[1]{',A1,           &
                     'begin{figure}','[h]')
              CALL DPWRST('XXX','WRIT')
 ...
 3187           FORMAT(23X,A1,'epsfig{file=#1,angle=-90,width=6.0in}')
                CALL DPWRST('XXX','WRIT')
              ELSE
                WRITE(ICOUT,33187)IBASLC
33187           FORMAT(23X,A1,'epsfig{file=#1,width=6.0in}')
                CALL DPWRST('XXX','WRIT')
              ENDIF
